...
object PicklePrinter {
  ...
  private final case class PickleEntry(num: Int, startIndex: Int, tag: Int, bytes: Array[Byte]) {
    override def toString = s"$num,$startIndex: ${tag2string(tag)}"
  }

  private final case class PickleEntries(entries: IndexedSeq[PickleEntry]) {
