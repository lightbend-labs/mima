...
object TastyUnpickler {
  ...
  abstract private class ClassTraverser(pkgInfo: PackageInfo) extends Traverser {
  ...
  def unpickleTree(in: TastyReader, names: Names): Tree = {
    ...
    def readTypeRefPkg() = TypeRefPkg(
      readName()
    ) // fullyQualified_NameRef          -- A reference to a package member with given fully qualified name
    def readTypeRef() = TypeRef(
      name = readName(),
      qual = readType()
    ) // NameRef qual_Type               -- A reference `qual.name` to a non-local member
    def readAnnot() = {
      readEnd(); Annot(readType(), skipTree(readByte()))
    } // tycon_Type fullAnnotation_Tree  -- An annotation, given (class) type of constructor, and full application tree
    def readSharedType() = unpickleTree(forkAt(readAddr()), names) match {
      case t @ UnknownTree(tag) =>
        throw new Exception(
          s"Expected Type, but was UnknownTree(${astTagToString(tag)}); init:${t.stack.map("\n\tat " + _).mkString}\n"
        )
    ...
    def readTree(): Tree = {
      ...
      def processLengthTree() = {
        ...
        def readPackage() = {
          val end = readEnd(); Pkg(readPath(), readTrees(end))
        } // Path Tree* -- package path { topLevelStats }
        ...
        def readTypeMemberDef(end: Addr) = {
          goto(end); UnknownTree(TYPEDEF)
        ...
        def readMods(end: Addr): (Option[Type], List[Annot]) = {
          ...
          doUntil(end)(readByte() match {
            ...
            case tag =>
              assert(false, s"illegal modifier tag ${astTagToString(tag)} at ${currentAddr.index - 1}, end = $end")
          })
  ...
  final case class Pkg(path: Path, trees: List[Tree]) extends Tree {
    def show = s"package $path${trees.map("\n  " + _).mkString}"
  }

  final case class ClsDef(name: TypeName, template: Template, privateWithin: Option[Type], annots: List[Annot])
      extends Tree {
  ...
  sealed class Traverser {
    ...
    def traverseClsDef(clsDef: ClsDef) = {
      val ClsDef(name, tmpl, privateWithin, annots) = clsDef; traverseName(name); traverseTemplate(tmpl);
      traversePrivateWithin(privateWithin); annots.foreach(traverse)
    }
    def traverseTemplate(tmpl: Template) = {
      val Template(classes, fields, meths) = tmpl; classes.foreach(traverse); fields.foreach(traverse);
      meths.foreach(traverse)
    }
    def traverseValDef(valDef: ValDef) = {
      val ValDef(name, privateWithin, annots) = valDef; traverseName(name); traversePrivateWithin(privateWithin);
      annots.foreach(traverse)
    }
    def traverseDefDef(defDef: DefDef) = {
      val DefDef(name, privateWithin, annots) = defDef; traverseName(name); traversePrivateWithin(privateWithin);
      annots.foreach(traverse)
    }
    def traversePrivateWithin(privateWithin: Option[Type]) = privateWithin.foreach(traverseType)
    def traverseAnnot(annot: Annot) = {
      val Annot(tycon, fullAnnotation) = annot; traverse(tycon); traverse(fullAnnotation)
    }
  ...
  private def readNewName(in: TastyReader, names: ArrayBuffer[Name]): Name = {
    ...
    def nameAtIdx(idx: Int) = try names(idx)
    catch {
    ...
    def readSignedRest(orig: Name, target: Name) =
      SignedName(
        orig,
        new MethodSignature(result = ErasedTypeRef(readName()), params = until(end)(readParamSig())),
        target
      )

    val name = tag match {
      case UTF8 =>
        val start = currentAddr; goto(end); SimpleName(new String(bytes.slice(start.index, end.index), "UTF-8"))
      ...
      case UNIQUE =>
        UniqueName(sep = readName().asSimpleName, num = readNat(), qual = ifBefore(end)(readName(), Name.Empty))
      ...
      case _ =>
        sys.error(s"at NameRef(${names.size}): name `${readName().debug}` is qualified by tag ${nameTagToString(tag)}")
    }
  ...
  object ErasedTypeRef {
    ...
    def apply(tname: Name): ErasedTypeRef = {
      ...
      def specialised(qual: Name, terminal: String, isModule: Boolean, arrayDims: Int = 0): ErasedTypeRef =
        terminal match {
  ...
  object DebugEncoder extends StringBuilderEncoder {
    def traverse(sb: StringBuilder, name: Name): StringBuilder = name match {
      ...
      case SignedName(name, sig, target) =>
        traverse(sb, name) ++= "[Signed (" ++= sig.map(_.signature).show ++= " @" ++= target.source += ']'
