...
final class TastyReader(val bytes: Array[Byte], start: Int, end: Int, val base: Int = 0) {
  ...
  def nextByte: Int =
    bytes(bp) & 0xff // Returns the next byte of data as a natural number without advancing the read position
  def readNat(): Int =
    readLongNat().toInt // Read a  natural number fitting in an Int in big endian format, base 128. All but the last digits have bit 0x80 set.
  def readInt(): Int =
    readLongInt().toInt // Read an integer number    in 2's complement big endian format, base 128. All but the last digits have bit 0x80 set.
  def readAddr(): Addr = Addr(readNat()) // Read a natural number and return as an address
  def readEnd(): Addr = addr(
    readNat() + bp
  ) // Read a length number and return the absolute end address implied by it, given as (address following length field) + (length-value-read)
