...
import com.typesafe.tools.mima.core.ui.widget.NavigationPanel
...
class Wizard extends BorderPanel {
  ...
  private val centerPane = new BorderPanel {
    ...
    private def worker(task: => Unit)(onEDT: => Unit): Actor = {
      import com.typesafe.tools.mima.core.ui.widget.BugReportDialog
      new Actor {
        def act() = {
          try {
            task
          } catch {
            case t: Throwable => new BugReportDialog(t)
          }
          Swing onEDT { onEDT }
        }
      }
