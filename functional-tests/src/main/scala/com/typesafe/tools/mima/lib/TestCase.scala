...
final class TestCase(val baseDir: Directory, val scalaCompiler: ScalaCompiler, val javaCompiler: JavaCompiler) {
  ...
  @tailrec private def rootCause(x: Throwable): Throwable = x match {
    case _: ExceptionInInitializerError | _: java.lang.reflect.InvocationTargetException |
        _: java.lang.reflect.UndeclaredThrowableException | _: java.util.concurrent.ExecutionException
        if x.getCause != null =>
